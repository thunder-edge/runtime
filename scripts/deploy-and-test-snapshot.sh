...
# Deploy SNAPSHOT bundles and run load tests
# Note: Currently uses ESZIP format as fallback until deno_core supports dynamic snapshots
...
BUNDLE_DIR="$PROJECT_ROOT/bundles/snapshot"

echo "🚀 SNAPSHOT Bundle Load Test (ESZIP fallback)"
echo "════════════════════════════════════════════════════════════"
echo "ℹ️  These bundles have snapshot headers but use ESZIP content"
echo "    Full performance gains will come with deno_core updates"
echo "════════════════════════════════════════════════════════════"
echo ""
...
if [ ! -d "$BUNDLE_DIR" ]; then
    ...
    echo "   Please run: ./scripts/bundle-snapshot.sh"
    exit 1
fi
...
# Deploy SNAPSHOT bundles
echo "📦 Deploying SNAPSHOT bundles..."
echo "────────────────────────────────────────────────────────────"
...
for bundle in "$BUNDLE_DIR"/*.pkg; do
    if [ -f "$bundle" ]; then
        ...
        response=$(curl -s -w "\n%{http_code}" -X POST "$SERVER_URL/_internal/functions" \
            ...
            -H "x-function-name: $example_name-snapshot" \
            --data-binary "@$bundle")
...
echo ""
echo "════════════════════════════════════════════════════════════"
echo "✅ SNAPSHOT load test completed"
echo ""
